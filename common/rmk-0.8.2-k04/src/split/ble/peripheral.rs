...
>(
    ...
    if central_addr.is_some() {
        ...
        send_controller_event(&mut controller_pub, ControllerEvent::SplitCentralUnpaired);
    }
    ...
    let peri_task = async {
        ...
        loop {
            ...
            if poll_clear_peer(&mut controller_sub) {
                clear_saved_central_peer(storage, &mut central_saved, &mut central_addr, &mut controller_pub).await;
            }
            ...
            let advertise_result = match select(
                ...
                wait_for_clear_peer(&mut controller_sub),
            )
            .await
            {
                Either::First(result) => result,
                Either::Second(()) => {
                    clear_saved_central_peer(storage, &mut central_saved, &mut central_addr, &mut controller_pub)
                        .await;
                    continue;
                }
            };
            #[cfg(not(all(feature = "controller", feature = "storage")))]
            let advertise_result = split_peripheral_advertise(id, central_addr, &mut peripheral, &server).await;
            ...
            match advertise_result {
                Ok(conn) => {
                    ...
                    let peer_addr = conn.raw().peer_address().into_inner();
                    if central_saved && Some(peer_addr) != central_addr {
                        warn!("Rejecting non-paired central address");
                        drop(conn);
                    ...
                    if !central_saved {
                        ...
                        {
                            central_saved = true;
                            central_addr = Some(peer_addr);
                        }
                    }
                    // Start run peripheral service
                    #[cfg(all(feature = "controller", feature = "storage"))]
                    let clear_peer = match select3(storage.run(), peripheral.run(), wait_for_clear_peer(&mut controller_sub))
                        .await
                    {
                        Either3::First(_) => false,
                        Either3::Second(clear_peer) => clear_peer,
                        Either3::Third(()) => true,
                    };
                    #[cfg(not(all(feature = "controller", feature = "storage")))]
                    let clear_peer = matches!(select(storage.run(), peripheral.run()).await, Either::Second(true));
...
#[cfg(all(feature = "controller", feature = "storage"))]
async fn wait_for_clear_peer(controller_sub: &mut crate::channel::ControllerSub) {
    loop {
        if let ControllerEvent::ClearPeer = controller_sub.next_message_pure().await {
            break;
        }
    }
}
...
#[cfg(all(feature = "controller", feature = "storage"))]
async fn clear_saved_central_peer<
    F: NorFlash,
    const ROW: usize,
    const COL: usize,
    const NUM_LAYER: usize,
...
>(
    storage: &mut Storage<F, ROW, COL, NUM_LAYER, NUM_ENCODER>,
    central_saved: &mut bool,
    central_addr: &mut Option<[u8; 6]>,
    ...
    *central_addr = None;
    CONNECTION_STATE.store(false, core::sync::atomic::Ordering::Release);
    send_controller_event(controller_pub, ControllerEvent::SplitCentralUnpaired);
}
...
async fn split_peripheral_advertise_until_connected<'a, 'b, C: Controller>(
    ...
    if central_addr.is_some() {
        ...
        match with_timeout(Duration::from_secs(10), advertiser.accept()).await {
            ...
            Err(_) => {
                warn!("[adv] directed central reconnect timeout, falling back to discoverable split adv");
            }
        }
    }
